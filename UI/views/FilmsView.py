"""Раздел «Фильмы и сериалы» — без ИИ, только RuTube и веб-поиск."""
...
class FilmsView(VideoView):
    ...
    def __init__(self, session, app):
        ...
        self._intent_box = ft.Container(visible=False, bgcolor=ft.Colors.TRANSPARENT)

    def _build_shell(self) -> None:
        self._search_field = SearchField(
            on_search=self.search,
            placeholder="Название фильма, сериала или ссылка",
    ...
    def _render(self, items, heading: str) -> None:
        ...
        if not items:
            self._set_results(
                EmptyState(
                    f"Ничего не найдено по запросу «{self._query}»" if self._query else "Каталог недоступен",
                    "Мы искали на RuTube и в интернете, но ничего подходящего не нашли. Попробуйте изменить формулировку.",
        ...
        if "web" in sources:
            note.append(StatusChip("Часть результатов из интернета — на RuTube их нет", COLORS["warning"], ft.Icons.TRAVEL_EXPLORE_ROUNDED))

        self._set_results(
            SectionTitle(heading, action_text="К подборкам" if self._query else "Обновить", on_action=self.load_feed if self._query else self._retry),
            *note,
    ...
    def _show_intent(self) -> None:
        ...
        if intent.genres:
            chips.append(StatusChip(", ".join(intent.genres), COLORS["gradient2"], ft.Icons.THEATER_COMEDY_ROUNDED))
        if intent.year_from:
        ...
        if intent.min_rating:
            chips.append(StatusChip(f"рейтинг от {intent.min_rating}", COLORS["muted"], ft.Icons.STAR_ROUNDED))
        if intent.content_type:
            chips.append(StatusChip("сериалы" if intent.content_type == "series" else "фильмы", COLORS["muted"], ft.Icons.LIVE_TV_ROUNDED))
        ...
        header = ft.Row(
            controls=[ft.Icon(ft.Icons.SEARCH_ROUNDED, size=16, color=COLORS["gradient1"]), ft.Text("Запрос разобран так:", size=13, color=ft.Colors.WHITE)],
            spacing=8,
        )
        ...
        self._intent_box.content = ft.Container(
