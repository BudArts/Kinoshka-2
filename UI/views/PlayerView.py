"""Экран просмотра — без серых контейнеров, без flet_video."""
...
class PlayerView(BaseView):
    ...
    def _render(self, item: MediaItem, stream_url, is_local: bool = False) -> None:
        ...
        player_box = self._build_player(item, stream_url)
    ...
    def _build_player(self, item: MediaItem, stream_url: Optional[str]) -> ft.Control:
        # Упрощённый плеер без Image, чтобы не было серых артефактов загрузки
        return ft.Container(
            ...
            content=ft.Column(
                controls=[
                    ...
                    ft.Text(item.title, size=16, color=ft.Colors.WHITE, text_align=ft.TextAlign.CENTER, max_lines=2, width=500),
                    ft.Text("Откройте в браузере или скачайте", size=13, color=ft.Colors.with_opacity(0.85, ft.Colors.WHITE)),
                ],
    ...
    def _info_block(self, item: MediaItem) -> ft.Control:
        ...
        controls: List[ft.Control] = [
            ...
            ft.Container(content=ft.Row(chips, spacing=8, wrap=True), bgcolor=ft.Colors.TRANSPARENT),
            ft.Container(content=ft.Row(actions, spacing=10, wrap=True), bgcolor=ft.Colors.TRANSPARENT),
        ]
        ...
        return ft.Container(content=ft.Column(controls=controls, spacing=14, tight=True), bgcolor=ft.Colors.TRANSPARENT)
    ...
    def _related_placeholder(self, item: MediaItem) -> ft.Control:
        ...
        return ft.Container(content=self._related_box, bgcolor=COLORS["bg"], padding=ft.Padding(0, 10, 0, 0))

    def _load_related(self, item: MediaItem) -> None:
        def render(items):
            if isinstance(items, Exception) or not items:
                self._related_box.controls = [
                    ft.Container(content=ft.Text("Похожих видео не нашлось", size=13, color=COLORS["muted"]), padding=12, bgcolor=COLORS["surface"], border_radius=10)
                ]
            else:
